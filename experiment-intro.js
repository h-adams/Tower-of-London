...
var problem_num_array = [];
if (my_var_1.indexOf(",") === -1) {
  problem_num_array = [parseInt(my_var_1)-1]; //sub 1 since arrays start at 0 but humans start at 1
} else {
  var problem_num_array = my_var_1.split(",");
  for (cnt = 0; cnt < problem_num_array.length; cnt++) {
    problem_num_array[cnt] = parseInt(problem_num_array[cnt])-1; //sub 1 since arrays start at 0 but humans start at 1
  }
}

console.log("problem array below");
console.log(problem_num_array);

var plural = problem_num_array.length === 1 ? "" : "s";
...
var problems = [];
for (cnt = 0; cnt<problem_num_array.length; cnt++) {
  problems.push(full_problems[problem_num_array[cnt]]);
}

console.log("problems array next");
...
var feedback_instruct_text =
  'Welcome. Press <strong>enter</strong> to begin.'
...
var start_test_block = {
  ...
  text: '<div class = centerbox><p class = block-text>We will now start Problem 1. There will be ' +
    problems.length + ' problem'+plural+' to complete. Press <strong>enter</strong> to begin.</p></div>',
  cont_key: [13],
  timing_post_trial: 1000,
  on_finish: function() {
    exp_stage = 'test'
